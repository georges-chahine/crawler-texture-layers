...
protected:
    ...
    cv::Mat texture, texture_temp, texture_temp2;
    double resolution_;
    ...
    pcl::VoxelGrid<pcl::PointXYZ> voxelize(pcl::PointCloud<pcl::PointXYZ>::Ptr& temp_cloud, float leaf_size=0.01){
...
//        directions << 1, 0 ,0;
        ...
        for (int v=0; v<rows; v++){
            for (int u=0; u<cols; u++)
            {
                ...
                Eigen::Vector3i coordinates=sor.getGridCoordinates(searchPoint2.x,searchPoint2.y,0);

               // int idx=sor.getCentroidIndexAt();
                int idx=-1;
                if (searchPoint2.x <10 && searchPoint2.y <10 && searchPoint2.x >-10 && searchPoint2.y >-10)
                {
                    idx=sor.getCentroidIndexAt(coordinates);
                }
...
//kdtree.radiusSearch (searchPoint, radius, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0
    ...
    void tempfn(bool newData, Eigen::Matrix4d newPose){
        ...
        if (init && newData){
            ...
            pcl::PointCloud<pcl::PointXYZ>::Ptr inliers_pcl_acc_tmp (new pcl::PointCloud<pcl::PointXYZ>);
            pcl::PointCloud<pcl::PointXYZ>::Ptr outliers_pcl_acc_tmp (new pcl::PointCloud<pcl::PointXYZ>);

            std::cout<<"inliers size is "<<inliers_pcl_acc->size()<<std::endl;
            pcl::transformPointCloud (*inliers_pcl_acc, *inliers_pcl_acc_tmp, pose_inv);
            pcl::transformPointCloud (*outliers_pcl_acc, *outliers_pcl_acc_tmp, pose_inv);
            ...
            if (inliers_pcl_acc->size()>0){
                publish_colored_texture(texture,newPose,inliers_pcl_acc_tmp, mergedTexturePub,"green", false); //todo: replace pose by the stampedPose (newpose)
            }


            if (outliers_pcl_acc->size()>0){

                publish_colored_texture(texture,newPose,outliers_pcl_acc_tmp, mergedTexturePub,"red", true);
            }
